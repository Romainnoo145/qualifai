...
must_haves:
  truths:
    ...
    - 'Admin receives Dutch-language email containing company name and matched use case titles when a prospect requests a quote'
...
4. Apply the migration normally: `npx prisma migrate deploy` (this applies all pending migrations in the migrations directory). Then run `npx prisma generate` to sync the Prisma client.

5. Create `lib/readable-slug.ts` with two exported functions:
...
2. **server/routers/admin.ts** — Wire readableSlug auto-generation into `enrichProspect`:
   ...
   - After `const enriched = await enrichCompany(...)` and before `ctx.db.prospect.update(...)`, check: if `enriched.companyName` is truthy and the current `prospect.readableSlug` is null, call `const readableSlug = await generateUniqueReadableSlug(ctx.db, enriched.companyName)` and determine `const shouldAutoSlug = enriched.companyName && !prospect.readableSlug`.
   - Change the update call from `data: buildEnrichmentData(enriched)` to `data: { ...buildEnrichmentData(enriched), ...(shouldAutoSlug ? { readableSlug } : {}) }` — spread alongside the helper output, do NOT modify `buildEnrichmentData()` itself.
