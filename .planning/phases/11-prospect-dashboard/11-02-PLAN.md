...
4. Session tracking — CRITICAL: Use `prospectSlug` (nanoid), NOT the readableSlug from the URL:
   - `api.wizard.startSession` on mount with `{ slug: prospectSlug, userAgent: navigator.userAgent }`
   - `api.wizard.trackProgress` on step change with sessionId, currentStep, stepTimes
   - `api.wizard.trackPdfDownload` on PDF download with sessionId
   - `api.wizard.trackCallBooked` on Cal.com click with sessionId
