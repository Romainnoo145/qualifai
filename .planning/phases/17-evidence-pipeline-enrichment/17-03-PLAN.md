...
must_haves:
  truths:
    ...
    - 'Apollo-derived LinkedIn profile data becomes an evidence draft when any Apollo data exists (description, specialties, or industry alone)'
...
4. **CRITICAL ORDERING — Read existingSnapshot for sitemap cache BEFORE the run create/update block.**

   The existing code at lines ~103-130 creates or updates the research run and overwrites `inputSnapshot`. If we read the prior snapshot AFTER that block, it will already be overwritten. Therefore, read it EARLY — right after the prospect is loaded, before any run create/update.

   Add this block **immediately after the prospect select query** (before the run create/update at lines ~103-130):
...
5. Replace the existing URL construction block (lines ~132-142):
   ...
   With sitemap-first approach (note: sitemapCache was already read in step 4 above):

   ```typescript
   // Sitemap discovery — always runs (zero API cost)
   // Cache was pre-read before run create/update (step 4)
...
6. Update the `inputSnapshot` writes to include `sitemapCache`. There are two places where `inputSnapshot` is written (create and update). Both need to include `sitemapCache` when sitemap was freshly discovered:
   ...
   IMPORTANT: The `priorSnapshot` read for sitemap cache happens BEFORE the run create/update (step 4), so it reads the PREVIOUS run's snapshot before it gets overwritten. For new runs (no `existingRunId`), there's no prior snapshot, so cache is always null — that's correct. For re-runs, the prior snapshot from `existingRunId` is read early and preserved in local variables.
   ...
   Confirm sitemap-first behavior: verify that `discoverSitemapUrls` is called in the URL construction path and that `defaultResearchUrls` is only used as fallback when `sitemapUrls.length === 0`. Also verify that `priorSnapshot` is read BEFORE the run create/update block (not after).
     </verify>
     <done>
