...
<task type="auto">
  ...
  <action>
    ...
    Props: `{ prospectId: string; signals?: any[] }`

    Use `api.research.listEvidence.useQuery({ prospectId })` to fetch all evidence items for this prospect. Accept signals as a prop — the page passes `p.signals` from the existing `api.admin.getProspect` query.
