...
**C. Update ActionRow for draft type:**

In the `ActionRow` component, add conditional rendering for `type === 'draft'`:

1. Show a truncated content preview below the title:
...
2. For draft items, replace the existing link (`/admin/outreach`) with an inline Send button:

```tsx
{item.type === 'draft' ? (
  <button
    onClick={() => approveDraft.mutate({ id: item.id })}
    disabled={approveDraft.isPending}
    className={cn(
      'ui-tap flex items-center gap-2 px-5 py-2 text-[10px] font-black uppercase tracking-widest rounded-full border transition-all',
  ...
  /* existing link behavior for hypothesis/task/reply items */
)}
```

3. Import `Send` and `Loader2` from `lucide-react` (add to existing imports).

4. The `approveDraft` mutation receives `{ id: item.id }` — no subject/bodyHtml overrides (uses existing draft content as-is for one-click send).

5. Add error handling: if `approveDraft` returns CONFLICT error (idempotency guard), show it gracefully. Use `onError` callback on the mutation to show a toast or inline error, OR use the existing pattern in the codebase. Simplest: add `onError` that calls `utils.admin.getActionQueue.invalidate()` so the stale row disappears on refresh.
...
**D. Add engagement indicator:**

For items with `engagementAt`, add a small indicator:
...
This goes in the ActionRow next to the timeAgo display.

**E. Keep non-draft items linking to their existing destinations:**
...
Only draft items get the inline send button.
</action>
