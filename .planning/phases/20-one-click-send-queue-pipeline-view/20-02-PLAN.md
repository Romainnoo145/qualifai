...
<PipelineChip
  stage={computePipelineStage({
    ...
    hasBookedSession: (p.sessions?.length ?? 0) > 0,
  })}
/>
```
...
Note: `getProspect` might not include `sessions` with `callBooked` filter. If `p.sessions` is not available, use `false` for `hasBookedSession` — the detail page will show approximate stage. This is acceptable because the detail page has full context visible. If `getProspect` already includes sessions data, use it.
</action>
