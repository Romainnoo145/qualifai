...
<verify>Run `npx tsc --noEmit` — no type errors. Navigate to a prospect detail page with hypotheses — cards show matched use cases with scores and evidence snippets. Approve/reject buttons still work.</verify>
<done>HypothesesTab displays matched use cases with confidence scores and supporting evidence snippets for both bottlenecks and automation opportunities. Evidence items link to source URLs. Cards are visually clean with truncation.</done>
</task>
