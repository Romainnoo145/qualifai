...
- Call `api.admin.getActionQueue.useQuery()` as the primary data source
- Also call `api.admin.getDashboardStats.useQuery()` for the summary KPI strip (keep total companies, reply rate — these are useful context)
...
3. **Action queue sections** — Four sections, each rendered only if items exist for that type. Each section:
   ...
   **Section: Drafts to Approve** (type === 'draft')
   ...
   - Link: `/admin/outreach` (all drafts are managed on the outreach page)
   ...
   **Section: Tasks Due** (type === 'task')
   ...
   - Link: `/admin/outreach` (tasks are managed on outreach page task tab)
   ...
   **Section: Replies to Handle** (type === 'reply')
   ...
   - Link: `/admin/outreach` (replies are managed on outreach page replies tab)
...
5. **Summary KPI strip** (below the queue, always visible) — compact row (grid grid-cols-2 lg:grid-cols-4 gap-3 mt-8 pt-8 border-t border-slate-100). Show from getDashboardStats:
   - Total Companies (data.total)
   - Active Signals (data.unprocessedSignals)
   - Reply Rate (pipeline.replied / pipeline.sent \* 100)%
   - Emails Sent (pipeline.sent)
     Each is a minimal text block (no card): count (text-lg font-bold text-[#040026]) + label (text-[10px] font-bold text-slate-400 uppercase tracking-widest).
...
- Import only needed lucide icons: Lightbulb, Mail, Phone, MessageSquare, MessageCircle, Linkedin, CheckCircle2, Building2, Zap, TrendingUp, Send
  </action>
  <verify>
  ...
  <done>
  Dashboard at /admin shows: (1) count strip with per-type totals and urgency indicators, (2) grouped action items with direct links, (3) empty state when nothing is pending, (4) summary KPI strip for context.
  </done>
  </task>
