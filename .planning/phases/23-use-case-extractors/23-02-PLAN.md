...
**IMPORTANT:** Plan 23-01 also modifies `server/routers/use-cases.ts` and `app/admin/use-cases/page.tsx`. Since both plans run in Wave 1 (parallel), there is a file conflict. To handle this:

- This plan (23-02) should be written to work whether 23-01 has been applied or not.
- When adding the procedure, place it after the existing procedures at the bottom of the router object.
- When modifying the UI, add the codebase section below the existing buttons area.
- If 23-01's changes are already present (importFromVault exists), work alongside them. If not, the code should still be valid.
...
3. Below the header buttons row, add a compact "Analyze Codebase" section:
   ...
   - A button labeled "Analyze Codebase" with `Code2` icon from lucide-react. Disabled when path is empty or mutation is pending.
